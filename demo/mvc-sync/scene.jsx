 ...
 * ----------------------------------------------------------------------------
 */
class SyncSelector extends BaseSelector {
  constructor() {
    super();
    // 动态添加Model到DB
    this.getAppStore().addModel(SyncModel)
  }

  get inputs() {
    return (state, ownProps) => {
      return {
        list: state.syncModel.list
      }
    }
  }
  ...
  get outputs() {
    return (dispatch, ownProps) => ({
      fetch: () => this.getModel('syncModel').fetch()(this.dispatch)
    });
  }

  initialize(ownProps) {
    this.getModel('syncModel').fetch()(this.dispatch);
  }
}
...
export default class SyncScene extends Component {
  static routePath = '/mvc-sync';

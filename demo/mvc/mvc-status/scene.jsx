 ...
 * ----------------------------------------------------------------------------
 */
class StatusSelector extends BaseSelector {
  constructor(){
    super();
    // 动态添加Model到DB
    this.getAppStore().addModel(StatusModel)
  }
  ...
  get outputs() {
    return (dispatch, ownProps) => ({
      fetch: () => this.getModel('statusModel').fetch()(this.dispatch)
    });
  }

  initialize(ownProps) {
    this.getModel('statusModel').fetch()(this.dispatch);
  }
}
...
export default class StatusScene extends Component {
  static routePath = '/mvc/mvc-status';
